...
class RnnReferenceModel(nn.Module):
    ...
    def __init__(self, word_dict, args):
        ...
        self.num_ref_predictor = nn.Sequential(
            nn.Linear(args.nhid_lang, args.nhid_lang),
            nn.Tanh(),
            nn.Dropout(args.dropout),
            #nn.Linear(args.nhid_lang, args.nhid_lang),
            #nn.Tanh(),
            #nn.Dropout(args.dropout),
            torch.nn.Linear(args.nhid_lang, 8)
            )
        ...
        init_cont(self.num_ref_predictor, args.init_range)
    ...
    def predict_num_referents(self, ctx_h, outs_emb, ref_inpt):
        if ref_inpt is None:
            return None
        ...
        #ctx_h_mean = ctx_h.mean(dim=1).unsqueeze(0).expand(ref_inpt.size(0), ctx_h.size(0), ctx_h.size(2))
        #num_ref_out = self.num_ref_predictor(torch.cat([ref_inpt, ctx_h_mean], 2))

        num_ref_out = self.num_ref_predictor(ref_inpt)
    ...
    def forward(self, ctx, inpt, ref_inpt, sel_idx):
        ...
        ref_out = self.reference_resolution(ctx_h, outs_emb, ref_inpt)
         ...
         # predict number of referents
        num_ref_out = self.predict_num_referents(ctx_h, outs_emb, ref_inpt)

        # compute selection
        sel_out = self.selection(ctx_h, outs_emb, sel_idx)

        return outs, ref_out, num_ref_out, sel_out
