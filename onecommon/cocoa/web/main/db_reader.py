...
class DatabaseReader(object):
    ...
    def dump_chats(cls, cursor, scenario_db, json_path, uids=None, accepted_only=False):
        ...
        if accepted_only:
            cursor.execute('SELECT DISTINCT chat_id FROM review WHERE accept=1')
            ids = cursor.fetchall()
    ...
    @classmethod
    def get_chat_duration(cls, cursor, chat_id, scenario_db, include_meta=False):
        events = cls.get_chat_events(cursor, chat_id, include_meta)
        return float(events[-1]['time']) - float(events[0]['time'])
